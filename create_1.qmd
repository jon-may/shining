...
As an example, we will create an app that asks for a file of data, and then draws a scatterplot and reports a regression. You can then play with it to try out different features.
...
```         
library(tidyverse)
write_csv(tibble(x = rnorm(1000,10,2), y = rnorm(1000,10,2)*.3+.7*x),"random.csv")
```

This makes a CSV file in your working directory called **random.csv** with 1000 rows, where **x** is a random value with a mean of 10 and a standard deviation of 2, and **y** is another random value that correlates highly with x.
...
When you run the app and drop the random.csv file into the Data slot you should see a scatterplot appear.
...
The last line draws the scatterplot.

Whatever is computed by the last line in a function is returned as the result of that function, so the scatterplot is rendered as **distPlot** and shown on the **ui**.

If you rerun the code that creates random.csv to get new values (maybe edit the weights of x an y?) then you can drop the new file into the Data slot and get a new scatterplot.

Maybe you could take a moment to edit the title so it does not claim to be Old Faithful Geyser Data.

This code will work on any csv file that contains columns called x and y, and it is a bit fragile, but it's your first real shiny app.
